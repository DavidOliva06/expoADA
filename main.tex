...
% ================================================================================
\begin{frame}
\frametitle{Introducción y fundamentos}
\framesubtitle{¿Qué es una metaheurística?}
\begin{itemize}
 \item El término metaheurística fue propuesto por Fred Glover en 1989 para definir métodos que guían y mejoran las heurísticas tradicionales.
  \item Son estrategias de optimización de alto nivel aplicadas a problemas complejos, donde los métodos exactos no son eficientes.
  \item Aunque no aseguran la solución óptima, permiten obtener resultados de calidad en menos tiempo.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Metaheurísticas iterativas}
\begin{itemize}
 \item Las metaheurísticas iterativas son métodos que mejoran una sola solución de forma repetitiva, aplicando pequeños cambios para obtener resultados de mayor calidad.
  \item Su objetivo es encontrar buenas soluciones sin explorar todo el espacio de búsqueda, evitando quedarse atrapadas en mínimos locales.
  \item Ejemplos: Recocido Simulado, Búsqueda Tabú,
   Búsqueda Local Iterada.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Origen y aplicaciones del ILS}
\begin{itemize}
 \item El ILS fue propuesto por Lourenço, Martin y Stützle en 2003 como una mejora de la búsqueda local tradicional.
  \item Su objetivo principal es superar los mínimos locales aplicando pequeñas perturbaciones que permiten explorar nuevas soluciones.
  \item Se aplicó inicialmente al Problema del Viajante, logrando resultados eficientes frente a métodos más costosos.
  \item Actualmente, se usa en diversos problemas de optimización combinatoria, como planificación, rutas, asignación de recursos y programación de tareas.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Importancia del ILS en la optimización combinatoria}
\begin{itemize}
 \item Es importante porque ofrece una forma eficiente y sencilla de resolver problemas de optimización combinatoria, donde el número de posibles soluciones es enorme y los métodos exactos resultan inviables.
  \item Combina búsqueda local y perturbaciones controladas para escapar de soluciones poco óptimas y explorar nuevas regiones del espacio de búsqueda.Gracias a ello se puede encontrar soluciones de alta calidad en menos tiempo.
  \item Su simplicidad, flexibilidad y buen rendimiento han hecho que el ILS sea una de las metaheurísticas más utilizadas y exitosas en áreas como la planificación, el enrutamiento y la asignación de recursos.
\end{itemize}
\end{frame}
% ================================================================================
...
\section{Estructura general del ILS}

\begin{frame}{Componentes principales del ILS}
  \begin{itemize}
    \item \textbf{Solución inicial:} punto de partida del algoritmo, puede ser aleatoria o heurística.
    \item \textbf{Búsqueda local:} Este procedimiento se aplica para alcanzar un óptimo local. \\
    
    \item \textbf{Perturbación:} modifica la solución actual para escapar de un óptimo local.
    \item \textbf{Criterio de aceptación:} determina si la búsqueda continúa desde la solución actual o desde el nuevo óptimo local encontrado.
  \end{itemize}

  \vspace{0.2cm}
  \begin{center}
    \fbox{\parbox{0.9\linewidth}{\centering \
    Diagrama del flujo ILS: Solución inicial → Búsqueda local → Perturbación → Aceptación → Repetir}}
  \end{center}
\end{frame}


%---------- Pseudocódigo básico -------------------
\begin{frame}{Pseudocódigo básico del ILS}
\small
\begin{algorithm}[H]
\caption{Iterated Local Search (básico)}
\begin{algorithmic}[1]
\State $s_0 \gets$ GenerarSoluciónInicial()
\State $s^* \gets$ BúsquedaLocal($s_0$)
\State $s_{best} \gets s^*$
\While{no se cumpla el criterio de parada}
  \State $s' \gets$ Perturbar($s^*$)
  \State $s^{*'} \gets$ BúsquedaLocal($s'$)
  \State $s^* \gets$ CriterioAceptación($s^*, s^{*'}$)
  \If{$f(s^*) < f(s_{best})$}
     \State $s_{best} \gets s^*$
  \EndIf
\EndWhile
...
\begin{frame}{Intensificación y diversificación}
  \begin{columns}
  \column{0.5\textwidth}
    \textbf{Intensificación:}
    \begin{itemize}
      \item Explorar a fondo una región prometedora.
      \item Se logra mediante la búsqueda local (mejorar soluciones cercanas).
      \item Aumenta la calidad de soluciones similares.
    \end{itemize}

  \column{0.5\textwidth}
    \textbf{Diversificación:}
    \begin{itemize}
      \item Explorar nuevas regiones del espacio de búsqueda.
      \item Se logra con perturbaciones más fuertes y el criterio de aceptación.
      \item Evita estancarse en óptimos locales.
    \end{itemize}
  \end{columns}

  \vspace{0.3cm}
  \begin{block}{Balance}
  Un buen ILS combina ambas estrategias para lograr una búsqueda eficiente y robusta.
  \end{block}
\end{frame}

%--------- Ejemplo ilustrativo --------
\begin{frame}{Ejemplo ilustrativo del funcionamiento de ILS}
  \small
  \begin{enumerate}
    \item \textbf{Paso 1 – Solución inicial:}  
    Se genera una ruta aleatoria entre 5 ciudades (A, B, C, D, E).  
    Ejemplo: A → B → C → D → E → A  
    \hfill Costo: 350 km

    \item \textbf{Paso 2 – Búsqueda local (Intensificación):}  
    Se intercambian nodos cercanos (2-opt) para mejorar la distancia.  
    Nueva ruta: A → B → D → C → E → A  
    \hfill Costo: 310 km

    \item \textbf{Paso 3 – Perturbación (Diversificación):}  
    Se altera el orden de varias ciudades para escapar del óptimo local.  
    Nueva ruta: A → D → B → E → C → A  
    \hfill Costo: 340 km

    \item \textbf{Paso 4 – Nueva búsqueda local:}  
    Desde esta nueva región, se encuentra una mejor solución.  
    Nueva ruta: A → D → C → E → B → A  
    \hfill Costo: 280 km

    \item \textbf{Paso 5 – Repetición:}  
    Se repite el ciclo de intensificación y diversificación hasta que no haya mejoras en la ruta.
  \end{enumerate}
\end{frame}
